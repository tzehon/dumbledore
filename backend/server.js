...
app.post('/api/communications', async (req, res) => {
    const db = await connectToDbForServer();
    const { userId, userType, templateId, trackingId } = req.body;
    ...
    const existingBucket = await db.collection('communications').findOne({ "user.id": userId, day: startOfDay });
    if (existingBucket && existingBucket.event_count >= 5) {
        return res.status(429).json({ message: "Frequency cap of 5 communications per day reached for this user." });
    }
    ...
    const mongoQuery = `db.getCollection('communications').updateOne({ "user.id": ${userId}, day: ISODate("${startOfDay.toISOString()}") }, { $push: { ... }, $inc: { ... }, $setOnInsert: { ... } }, { upsert: true })`;
